...
izmSearchUI <- function(id, 
  ...
  pagefun(
    ...
    tags$div(id = ns("form_ui_wrapper"), class = "form-container",
             tags$form(name = ns("izmsearch"),
                       fluidRow(
                         column(4,
                                izm_search_input("achternaam", "Achternaam", url, timeout, ns),  
                                izm_search_input("straatnaam", "Straatnaam", url, timeout, ns)
                         ),
                         column(4,
                                izm_search_input("bsn", "BSN", url, timeout, ns),
                                izm_search_input("huisnummer", "Huisnummer", url, timeout, ns),
                         ),
                         column(4,
                                izm_search_input("geboortedatum", "Geboortedatum", url, timeout, placeholder = 'dd-mm-jjjj/ddmmjjjj', ns),
                                izm_search_input("postcode", "Postcode", url, timeout, placeholder = '1234AA', ns)
                         )
                       ) 
             )
    ),
    ...
    tags$div(id = ns("searchresults_ui_wrapper"), class = "result-table-container", `data-tableclickedid` = ns("izmclickedid"),
             tags$table(id = ns("searchresults"), class = "display", style = "width: 100%")
...
#----- Utils ------
# not exported.
izm_search_input <- function(what, label, url, timeout, ns, placeholder = ''){
  ...
  tagList(        
    ...
    tags$input(
      id = ns(what),
      onkeyup = glue("autosearch('{url}', {timeout}, '{namespace}');"),
      type = "text",
