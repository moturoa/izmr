...
casusTijdlijnModule <- function(input, output, session, bronnen){
  ...
  output$persoon_tijdlijn <- renderUI({
    ...
    print(rv$data_bron)
    print(2)
    print(input$chk_bron_filters)
    print(3)
    print(input$daterange_tijdlijn[1])
    print(4)
    print(input$chk_nulldate)
    print(5)
    print(rv$data_bron %>% filter(bron %in% input$chk_bron_filters & (begindatum >= ymd(input$daterange_tijdlijn[1]) | input$chk_nulldate)))
    print(6)
...
make_timeline_persoon <- function(brondata){ 
  ...
  timeline_items <- lapply(persoon_data, function(x){
    print('XXXXXXXXX')
    print(x)
    print(x$omschrijving)
    print(x$bron)
    print(x$begindatum_formatted)
    print(x$icon) 
    print(x$color)
    print(7777)
